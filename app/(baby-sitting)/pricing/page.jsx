...
import { Check, Loader, RefreshCw, XCircle, ArrowUpCircle, Info } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10">
        <h3 className="text-lg font-bold font-poppins text-gray-800 mb-4">{title}</h3>
        {children}
      </div>
    </div>
  );
...
const SubscriptionPage = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/parent/subscription?role=${selectedRole}`);
  ...
  useEffect(() => { fetchUserState(); }, [fetchUserState]);
  ...
  const monthlyPlanObj = filteredPlans.find(p => p.name.toLowerCase() === 'monthly') || filteredPlans[0];
  const yearlyPlanObj = filteredPlans.find(p => p.name.toLowerCase() === 'yearly') || filteredPlans[1];
  ...
  const features = {
    parent:     ['Find trusted babysitters near you', 'UNLIMITED babysitter searches', 'Background checks & verified profiles', 'Cancellation protection'],
    babysitter: ['Get hired by families', 'Build your reputation', 'Profile visibility boost', 'Priority listing'],
  };
  ...
  const PlanButton = ({ billingCycle, planId, baseColor, hoverColor }) => {
      ...
      return (
        <div className="space-y-2">
          <div className="flex items-center gap-2 justify-center text-xs text-brandColor font-poppins font-medium bg-green-50 border border-green-200 py-2 px-3 rounded-xl">
            ...
            {activeSub?.endDate && (
              <span className="text-gray-400 ml-1">· renews {fmtDate(activeSub.endDate)}</span>
            )}
          </div>
    ...
    if (state === 'switch') {
      const switchLabel = billingCycle === 'yearly' ? 'Switch to Yearly' : 'Switch to Monthly';
      return (
        <button
          onClick={() => handlePlanClick(planId, billingCycle)}
        ...
        >
          {(checkoutLoading || cancelLoading) ? <Loader size={20} className="animate-spin" /> : <ArrowUpCircle size={20} />}
    ...
    if (isNew || isEligibleForNewTrial) {
      ...
      const remainingDays = Math.ceil((new Date(userProfile.subscriptionExpiry).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
      buttonLabel = `Trial Running (${remainingDays > 0 ? remainingDays : 0} Days Left)`;
    } else {
      buttonLabel = billingCycle === 'yearly' ? 'Choose Yearly' : 'Choose Monthly';
    }
  ...
  const isUserOnTrial = userProfile?.subscription === "trial" && userProfile?.subscriptionExpiry && new Date(userProfile.subscriptionExpiry) > new Date();
  const hasNeverHadTrial = userProfile && userProfile.subscription === "free" && !userProfile.subscriptionStart;
  ...
  return (
    <section className="min-h-screen pt-30 pb-12 lg:pt-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {isUserOnTrial && (
          <div className="mb-8 max-w-3xl mx-auto bg-green-50 border border-green-200 rounded-2xl px-5 py-4 flex items-start gap-3 shadow-xs">
            ...
            <div className="text-sm font-poppins text-gray-700">
              <span className="font-semibold text-brandColor">You are on a Free Trial!</span>{" "}
              Your free access is active until <strong className="text-gray-900">{fmtDate(userProfile.subscriptionExpiry)}</strong>.
              You will not be charged anything during this trial period. If you subscribe to a paid plan now, you will pay <strong className="text-gray-900">£0.00 today</strong>, and you will only be charged <strong className="text-gray-900">£2.99/month</strong> or <strong className="text-gray-900">£26.91/year</strong> when your trial ends, unless you cancel beforehand.
            </div>
          </div>
        )}
        ...
        {activeSub && (
          <div className="mb-8 max-w-3xl mx-auto bg-green-50 border border-green-200 rounded-2xl px-5 py-4 flex items-start gap-3">
            ...
            <div className="text-sm font-poppins text-gray-700">
              <span className="font-semibold text-brandColor">Active subscription:</span>{" "}
              {activeSub.plan} · {activeSub.billingCycle === "yearly" ? "Yearly" : "Monthly"} · Renews {fmtDate(activeSub.endDate)}
            </div>
          </div>
        )}
                ...
                <div className="flex flex-col items-center justify-center mb-4">
                  ...
                  {showTrialBadge && (
                    <span className="mt-2 inline-block px-3 py-1 bg-green-50 text-brandColor text-xs font-semibold rounded-full border border-green-200">
                      {selectedRole === "parent" ? "Includes 1 Month Free Trial" : "Includes 2 Months Free Trial"}
                    </span>
                  )}
                </div>
            ...
            <div>
              ...
              <div className="p-8 text-center pb-2">
                <div className="flex flex-col items-center justify-center mb-2">
                  <div className="flex items-center justify-center">
                    <span className="text-5xl font-bold text-gray-900">£{yearlyPrice.toFixed(2)}</span>
                    <span className="text-gray-500 text-lg ml-1">/year</span>
                  </div>
                  <p className="text-gray-500 text-sm mt-1">£{(yearlyPrice / 12).toFixed(2)} per month</p>
                  {showTrialBadge && (
                    <span className="mt-2 inline-block px-3 py-1 bg-green-50 text-brandColor text-xs font-semibold rounded-full border border-green-200">
                      {selectedRole === "parent" ? "Includes 1 Month Free Trial" : "Includes 2 Months Free Trial"}
                    </span>
      ...
      <ActionModal open={modal?.type === 'cancel' || modal?.type === 'current' && modal?.action === 'cancel'} onClose={() => setModal(null)} title="Cancel Subscription">
        <p className="text-sm text-gray-600 font-poppins mb-5">
          Are you sure you want to cancel your current subscription?
          {activeSub?.endDate && (
            <span className="block mt-1 text-gray-400">You will keep access until <strong>{fmtDate(activeSub.endDate)}</strong>.</span>
          )}
        </p>
        <div className="flex gap-3">
          <button onClick={() => setModal(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 text-sm font-poppins hover:bg-gray-50">
            Keep Plan
          </button>
          <button onClick={doCancel} disabled={cancelLoading} className="flex-1 py-2.5 rounded-xl bg-red-500 text-white text-sm font-poppins hover:bg-red-600 disabled:opacity-50 flex items-center justify-center gap-2">
            {cancelLoading ? <Loader size={14} className="animate-spin" /> : <XCircle size={14} />}
      ...
      <ActionModal open={modal?.type === 'current'} onClose={() => setModal(null)} title="Manage Your Subscription">
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-5 text-sm font-poppins text-gray-700">
          <p><span className="font-semibold text-brandColor">Active:</span> {activeSub?.plan} · {activeSub?.billingCycle === 'yearly' ? 'Yearly' : 'Monthly'}</p>
          {activeSub?.endDate && <p className="text-xs text-gray-400 mt-1">Renews on {fmtDate(activeSub.endDate)}</p>}
        </div>
        <div className="space-y-2">
          <button
            onClick={async () => { setModal(null); await doCheckout(modal.planId, modal.billingCycle); }}
            disabled={checkoutLoading}
          ...
          >
            {checkoutLoading ? <Loader size={14} className="animate-spin" /> : <RefreshCw size={14} />}
            Renew Plan
          </button>
          <button
            onClick={() => setModal({ type: 'cancel' })}
      ...
      {/* ─── Switch Plan Modal ─── */}
      <ActionModal open={modal?.type === 'switch'} onClose={() => setModal(null)} title="Switch Billing Cycle">
        <p className="text-sm text-gray-600 font-poppins mb-2">
          Your current plan will be <strong>cancelled</strong> and you'll be charged for the new{" "}
          <strong className="capitalize">{modal?.billingCycle}</strong> plan immediately.
        </p>
        {activeSub?.endDate && (
          <p className="text-xs text-gray-400 font-poppins mb-5">Current plan ends: {fmtDate(activeSub.endDate)}</p>
        )}
        <div className="flex gap-3">
          <button onClick={() => setModal(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 text-sm font-poppins hover:bg-gray-50">
            Keep Current
          </button>
          <button onClick={doSwitch} disabled={cancelLoading || checkoutLoading} className="flex-1 py-2.5 rounded-xl bg-brandColor text-white text-sm font-poppins hover:bg-[#558b2f] disabled:opacity-50 flex items-center justify-center gap-2">
            {(cancelLoading || checkoutLoading) ? <Loader size={14} className="animate-spin" /> : <ArrowUpCircle size={14} />}
